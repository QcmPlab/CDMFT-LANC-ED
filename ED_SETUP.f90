...
contains
   ...
   subroutine init_ed_structure()
      ...
      if(chispin_flag)then
         ...
         allocate(spinChi_w(Norb,Norb,0:Lreal))
      ...
      if(chidens_flag)then
         ...
         allocate(densChi_tot_w(0:Lreal))
   ...
   subroutine build_sector(isector,self,Itrace)
      ...
      enddo
      !
      ! Fill back all ther fields of the sector object
      self % index  = isector
      self % H      = H
      self % DimUps = DimUps
      self % DimDws = DimDws
      self % DimUp  = DimUp
      self % DimDw  = DimDw
      self % Dim    = DimUp*DimDw
      self % DimEl  = self % DimDw
      self % DimPh  = 0
      self % Nups   = Nups
      self % Ndws   = Ndws
      self % Nup    = sum(Nups)
   ...
   !> i=instate , j=outstate, ipos=site+orb index, ialfa=elicity, ispin, sectorI=sector in,sectorJ=sector out
