  subroutine ed_get_single_particle_density_matrix_lattice(dm,dm_eig,dm_rot,doprint)
    !passed
    ...
    !
    if(allocated(dm))deallocate(dm)                             ;allocate(dm(Nsites,Nlat*Nspin*Norb,Nlat*Nspin*Norb))   ;dm     = zero
    if(present(dm_eig).and.allocated(dm_eig))deallocate(dm_eig);allocate(dm_eig(Nsites,Nlat*Nspin*Norb))                ;dm_eig = zero
    if(present(dm_rot).and.allocated(dm_rot))deallocate(dm_rot);allocate(dm_rot(Nsites,Nlat*Nspin*Norb,Nlat*Nspin*Norb));dm_rot = zero
    !
    do isite=1,Nsites
       !
       ...
       if(doprint_)then
          if(present(dm_eig).and.present(dm_rot))then
            call print_sp_dm(dm(isite,:,:),dm_eig(isite,:),dm_rot(isite,:,:),ineq=isite)
          else
            call print_sp_dm(dm(isite,:,:),ineq=isite)
          endif
       endif
       !
    enddo
