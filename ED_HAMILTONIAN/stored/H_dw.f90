        ...
        do jlat=1,Nlat
          do iorb=1,Norb
            do jorb=1,Norb
              ...
              ibeta = getBathStride(jlat,jorb,kp)
              Jcondition = &
                   (dmft_bath%item(kp)%h(ilat,jlat,Nspin,Nspin,iorb,jorb)/=zero) &
              ...
              if (Jcondition)then
                ...
                jdw = binary_search(Hs(2)%map,k2)
                htmp = dmft_bath%item(kp)%h(ilat,jlat,Nspin,Nspin,iorb,jorb)*sg1*sg2
                    !
                call sp_insert_element(spH0dws(1),htmp,idw,jdw)
                !
              endif
