program cdn_kanemele
   ...
   allocate(Hsym_basis(Nlat,Nlat,Nspin,Nspin,Norb,Norb,5))
   !
   ...
   !
   !This is sort of a hack. setting the vector to 0d0 makes sure the local
   !hamiltonian does not have that term. the bath now has to be initialized
   !by hand in order for the additional parameters to be used
   lambdasym_vector(4)=0d0
   Hsym_basis(:,:,:,:,:,:,4)=lso2nnn(additional_bath_model(1.d0,0.d0))
   !
...
contains
   ...
   function additional_bath_model(t2_, t3_) result (bath_)
      ! Directly created in lso basis
      integer                                               :: ispin, lowerbound, upperbound, Nlo
      real(8),intent(in)                                    :: t2_,t3_
      ...
      do ispin=1,Nspin
         lowerbound = (ispin-1)*Nlo+1
         upperbound = ispin*Nlo
         bath_(lowerbound:upperbound,lowerbound:upperbound) = additional_bath_matrix(t2_, t3_)
      enddo
      !
   ...
   function additional_bath_matrix(t2, t3) result(bmat)
      complex(8),dimension(6,6) :: bmat
      real(8),dimension(6,6)    :: tempmat
      real(8),intent(in)        :: t2, t3
      !
      bmat    = zero
      tempmat = zero
      !
