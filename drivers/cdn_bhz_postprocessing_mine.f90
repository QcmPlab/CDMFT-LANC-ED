program cdn_bhz_2d
   ...
   !
   !Smats(1,2,:,:,1,2,:)=10*Smats(1,2,:,:,1,2,:)
   !Smats(1,2,:,:,2,1,:)=10*Smats(1,2,:,:,2,1,:)
   !Smats(2,1,:,:,1,2,:)=10*Smats(2,1,:,:,1,2,:)
   !Smats(2,1,:,:,2,1,:)=10*Smats(2,1,:,:,2,1,:)
   !
   !Sreal(1,2,:,:,1,2,:)=10*Sreal(1,2,:,:,1,2,:)
   !Sreal(1,2,:,:,2,1,:)=10*Sreal(1,2,:,:,2,1,:)
   !Sreal(2,1,:,:,1,2,:)=10*Sreal(2,1,:,:,1,2,:)
   !Sreal(2,1,:,:,2,1,:)=10*Sreal(2,1,:,:,2,1,:)
   !
...
contains
   ...
   subroutine print_hk_periodized_path()
      ...
      kpath(1,:)=kpoint_Gamma(1:2)
      kpath(2,:)=kpoint_X1(1:2)
      kpath(3,:)=kpoint_M1(1:2)
      kpath(4,:)=kpoint_X2(1:2)
      kpath(5,:)=kpoint_Gamma(1:2)
      ...
      call TB_set_bk([pi2,0d0],[0d0,pi2])
        call TB_Solve_model(hk_periodized,Nspin*Norb,kpath,Nkpath,&
         ...
         points_name=[character(len=20) :: 'G', 'X', 'M', 'Y', 'G'],&
         file=reg(file))
      !
  ...
  subroutine get_zeros()
    ...
    do int=1,maxNinterval
       ...
       if(any((Mpoles(:,int)/=0.d0)))then
          do ik=1,Nktot
             if(Mpoles(ik,int)/=0.d0)write(unit,*)kpoints(ik,1),kpoints(ik,2),Mpoles(ik,int),Mweight(ik,int)
          enddo
       endif
       close(unit)
    enddo
