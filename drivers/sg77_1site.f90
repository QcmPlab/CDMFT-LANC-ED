...
contains
  ...
  function hk_model(kpoint,N) result(Hk)
    ...
    !Hop 1 (other four terms are in Hloc)
      hopping_matrix(1,1,ispin,ispin,1,1) = hopping_matrix(1,1,ispin,ispin,1,1) + (ts/2d0)*exp(xi*kpoint(1)*1)
      hopping_matrix(1,1,ispin,ispin,1,1) = hopping_matrix(1,1,ispin,ispin,1,1) + (ts/2d0)*exp(-xi*kpoint(1)*1)
      hopping_matrix(1,1,ispin,ispin,2,2) = hopping_matrix(1,1,ispin,ispin,2,2) - (ts/2d0)*exp(xi*kpoint(1)*1)
      hopping_matrix(1,1,ispin,ispin,2,2) = hopping_matrix(1,1,ispin,ispin,2,2) - (ts/2d0)*exp(-xi*kpoint(1)*1)
    !Hop 2 (cosine)
    ...
    !Hop 3
      hopping_matrix(1,1,ispin,ispin,1,1) = hopping_matrix(1,1,ispin,ispin,1,1) - (ts/4d0)*exp(-xi*dot_product(kpoint,[1d0,1d0,0d0]))
      hopping_matrix(1,1,ispin,ispin,1,1) = hopping_matrix(1,1,ispin,ispin,1,1) - (ts/4d0)*exp(-xi*dot_product(kpoint,[-1d0,-1d0,0d0]))
      hopping_matrix(1,1,ispin,ispin,2,2) = hopping_matrix(1,1,ispin,ispin,2,2) + (ts/4d0)*exp(-xi*dot_product(kpoint,[1d0,1d0,0d0]))
      hopping_matrix(1,1,ispin,ispin,2,2) = hopping_matrix(1,1,ispin,ispin,2,2) + (ts/4d0)*exp(-xi*dot_product(kpoint,[-1d0,-1d0,0d0]))
    !Hop 4
      hopping_matrix(1,1,ispin,ispin,1,1) = hopping_matrix(1,1,ispin,ispin,1,1) + (ts/4d0)*exp(-xi*dot_product(kpoint,[-1d0,1d0,0d0]))
      hopping_matrix(1,1,ispin,ispin,1,1) = hopping_matrix(1,1,ispin,ispin,1,1) + (ts/4d0)*exp(-xi*dot_product(kpoint,[1d0,-1d0,0d0]))
    ...
    !Hop 5 (other 4 terms are in Hloc)
      hopping_matrix(1,1,ispin,ispin,1,2) = hopping_matrix(1,1,ispin,ispin,1,2) + (ts/4d0)*exp(xi*kpoint(1))
      hopping_matrix(1,1,ispin,ispin,1,2) = hopping_matrix(1,1,ispin,ispin,1,2) + (ts/4d0)*exp(-xi*kpoint(1)) 
      hopping_matrix(1,1,ispin,ispin,2,1) = hopping_matrix(1,1,ispin,ispin,2,1) + (ts/4d0)*exp(xi*kpoint(1))
      hopping_matrix(1,1,ispin,ispin,2,1) = hopping_matrix(1,1,ispin,ispin,2,1) + (ts/4d0)*exp(-xi*kpoint(1)) 
    !Hop 6
  ...
  function hk_2bands(kpoint,N) result(Hk)
    integer                                                      :: N,ispin
    real(8),dimension(:)                                         :: kpoint
    complex(8),dimension(N,N)                                    :: hk
    !
    hk=zero
    hk(1:2,1:2)=hk_model(kpoint,Nspin*Norb)
    hk(3:4,3:4)=hk_model(kpoint+[pi,0d0,0d0],Nspin*Norb)
    !
  ...
  subroutine print_hk()
      ...
      !call TB_Solve_model(hk_2bands,2*Nspin*Norb,kpath,500,&
      call TB_Solve_model(hk_2bands,2*Nspin*Norb,kpath,500,&
