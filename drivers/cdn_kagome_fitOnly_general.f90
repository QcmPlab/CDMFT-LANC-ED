program cdn_kagome
   ...
   !SETUP BATH STEP 1
   allocate(lambdasym_vector(9))
   allocate(Hsym_basis(Nlat,Nlat,Nspin,Nspin,Norb,Norb,9))
   !
   !lambdasym_vector(1)=ts
   !Hsym_basis(:,:,:,:,:,:,1)=lso2nnn(hloc_model(1.d0))
   !
   lambdasym_vector(1)=0d0
   Hsym_basis(:,:,:,:,:,:,1)=lso2nnn(bath_model(1.d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0))
   !
   lambdasym_vector(2)=0d0
   Hsym_basis(:,:,:,:,:,:,2)=lso2nnn(bath_model(0.d0, 1d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0))
   !
   lambdasym_vector(3)=0d0
   Hsym_basis(:,:,:,:,:,:,3)=lso2nnn(bath_model(0d0,  0d0, 1d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0))
   !
   lambdasym_vector(4)=0d0
   Hsym_basis(:,:,:,:,:,:,4)=lso2nnn(bath_model(0d0,  0d0, 0d0, 1d0, 0d0, 0d0, 0d0, 0d0, 0d0))
   !
   lambdasym_vector(5)=0d0
   Hsym_basis(:,:,:,:,:,:,5)=lso2nnn(bath_model(0d0,  0d0, 0d0, 0d0, 1d0, 0d0, 0d0, 0d0, 0d0))
   !
   lambdasym_vector(6)=0d0
   Hsym_basis(:,:,:,:,:,:,6)=lso2nnn(bath_model(0d0,  0d0, 0d0, 0d0, 0d0, 1d0, 0d0, 0d0, 0d0))
   !
   lambdasym_vector(7)=0d0
   Hsym_basis(:,:,:,:,:,:,7)=lso2nnn(bath_model(0d0,  0d0, 0d0, 0d0, 0d0, 0d0, 1d0, 0d0, 0d0))
   !
   lambdasym_vector(8)=0d0
   Hsym_basis(:,:,:,:,:,:,8)=lso2nnn(bath_model(0d0,  0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 1d0, 0d0))
   !
   lambdasym_vector(9)=0d0
   Hsym_basis(:,:,:,:,:,:,9)=lso2nnn(bath_model(0d0,  0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 1d0))
   !
   !lambdasym_vector(10)=0d0
   !Hsym_basis(:,:,:,:,:,:,10)=(0,1)*lso2nnn(bath_model(1.d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0))
   !!
   !lambdasym_vector(11)=0d0
   !Hsym_basis(:,:,:,:,:,:,11)=(0,1)*lso2nnn(bath_model(0.d0, 1d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0))
   !!
   !lambdasym_vector(12)=0d0
   !Hsym_basis(:,:,:,:,:,:,12)=(0,1)*lso2nnn(bath_model(0d0,  0d0, 1d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0))
   !!
   !lambdasym_vector(13)=0d0
   !Hsym_basis(:,:,:,:,:,:,13)=(0,1)*lso2nnn(bath_model(0d0,  0d0, 0d0, 1d0, 0d0, 0d0, 0d0, 0d0, 0d0))
   !!
   !lambdasym_vector(14)=0d0
   !Hsym_basis(:,:,:,:,:,:,14)=(0,1)*lso2nnn(bath_model(0d0,  0d0, 0d0, 0d0, 1d0, 0d0, 0d0, 0d0, 0d0))
   !!
   !lambdasym_vector(15)=0d0
   !Hsym_basis(:,:,:,:,:,:,15)=(0,1)*lso2nnn(bath_model(0d0,  0d0, 0d0, 0d0, 0d0, 1d0, 0d0, 0d0, 0d0))
   !!
   !lambdasym_vector(16)=0d0
   !Hsym_basis(:,:,:,:,:,:,16)=(0,1)*lso2nnn(bath_model(0d0,  0d0, 0d0, 0d0, 0d0, 0d0, 1d0, 0d0, 0d0))
   !!
   !lambdasym_vector(17)=0d0
   !Hsym_basis(:,:,:,:,:,:,17)=(0,1)*lso2nnn(bath_model(0d0,  0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 1d0, 0d0))
   !!
   !lambdasym_vector(18)=0d0
   !Hsym_basis(:,:,:,:,:,:,18)=(0,1)*lso2nnn(bath_model(0d0,  0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 0d0, 1d0))
   !
...
contains
   ...
   function bath_model(t1_, t2_, t3_, t4_, t5_, t6_, t7_, t8_, t9_) result (bath_)
      ! Directly created in lso basis
      integer                                               :: ispin, iorb, lowerbound, upperbound
      integer                                               :: Nlo
      real(8),intent(in)                                    :: t1_, t2_, t3_, t4_, t5_, t6_, t7_, t8_, t9_
      ...
      do ispin=1,Nspin
         lowerbound = (ispin-1)*Nlo+1
         upperbound = ispin*Nlo
         bath_(lowerbound:upperbound,lowerbound:upperbound) = bath_matrix(t1_, t2_, t3_, t4_, t5_, t6_, t7_, t8_, t9_)
      enddo
      !
   ...
   function bath_matrix(t1, t2, t3, t4, t5, t6, t7, t8, t9) result(bmat)
      ...
      tempmat(2,:) = [ 0, 0, 0]
      tempmat(3,:) = [ 0, 0, 0]
      bmat         = bmat + t1*tempmat
      !
      tempmat(1,:) = [ 0, 1, 0]
      tempmat(2,:) = [ 0, 0, 0]
      tempmat(3,:) = [ 0, 0, 0]
      bmat         = bmat + t2*tempmat
      !
      tempmat(1,:) = [ 0, 0, 1]
      tempmat(2,:) = [ 0, 0, 0]
      tempmat(3,:) = [ 0, 0, 0]
      bmat         = bmat + t3*tempmat
      !
      tempmat(1,:) = [ 0, 0, 0]
      tempmat(2,:) = [ 1, 0, 0]
      tempmat(3,:) = [ 0, 0, 0]
      bmat         = bmat + t4*tempmat
      !
      tempmat(1,:) = [ 0, 0, 0]
      tempmat(2,:) = [ 0, 1, 0]
      tempmat(3,:) = [ 0, 0, 0]
      bmat         = bmat + t5*tempmat
      !
      tempmat(1,:) = [ 0, 0, 0]
      tempmat(2,:) = [ 0, 0, 1]
      tempmat(3,:) = [ 0, 0, 0]
      bmat         = bmat + t6*tempmat
      !
      tempmat(1,:) = [ 0, 0, 0]
      tempmat(2,:) = [ 0, 0, 0]
      tempmat(3,:) = [ 1, 0, 0]
      bmat         = bmat + t7*tempmat
      !
      tempmat(1,:) = [ 0, 0, 0]
      tempmat(2,:) = [ 0, 0, 0]
      tempmat(3,:) = [ 0, 1, 0]
      bmat         = bmat + t8*tempmat
      !
      tempmat(1,:) = [ 0, 0, 0]
      tempmat(2,:) = [ 0, 0, 0]
      tempmat(3,:) = [ 0, 0, 1]
      bmat         = bmat + t9*tempmat
      !
