program cdn_nematic_2x2
   ...
   allocate(lambdasym_vectors(Nbath,4))
   allocate(Hsym_basis(Nlat,Nlat,Nspin,Nspin,Norb,Norb,4))
   Hsym_basis(:,:,:,:,:,:,1) = lso2nnn(zeye(Nlso)) !Replica onsite energies
   Hsym_basis(:,:,:,:,:,:,2) = lso2nnn(xnn_hoppings())
   Hsym_basis(:,:,:,:,:,:,3) = lso2nnn(ynn_hoppings())
   Hsym_basis(:,:,:,:,:,:,4) = lso2nnn(nnn_hoppings())
   ...
   do irepl=1,Nbath
      ...
      lambdasym_vectors(irepl,2) = ts         !Multiplies x nearest-neighbor hoppings
      lambdasym_vectors(irepl,3) = ts         !Multiplies y nearest-neighbor hoppings
      lambdasym_vectors(irepl,4) = 1d-3       !Multiplies the NNN hoppings (small)
   enddo
...
contains
   ...
   function xnn_hoppings() result(Hxnn)
     complex(8),dimension(Nlat,Nlat) :: Hxnn
     Hxnn=zero
     Hxnn(1,2)=one
     Hxnn(2,1)=one
     Hxnn(4,3)=one
     Hxnn(3,4)=one
   end function xnn_hoppings

   function ynn_hoppings() result(Hynn)
     complex(8),dimension(Nlat,Nlat) :: Hynn
     Hynn=zero
     Hynn(1,3)=one
     Hynn(3,1)=one
     Hynn(2,4)=one
     Hynn(4,2)=one
   end function ynn_hoppings
   
   function nnn_hoppings() result(Hnnn)
     complex(8),dimension(Nlat,Nlat) :: Hnnn
     Hnnn=zero
     Hnnn(1,4)=one
     Hnnn(4,1)=one
     Hnnn(2,3)=one
     Hnnn(3,2)=one
   end function nnn_hoppings
